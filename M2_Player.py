...
class Player(M1_Objects.Object):
    def __init__(self, img, id, anim_radius, weapon, cord=[M6_Constants.W // 2, M6_Constants.H // 2], angle=0, health=1500, speed=[0, 0], angle_speed=0, max_angle_speed=4, speed_boost=0.1):
        id = id + '.' + str(random.randint(9999999, 100000000))
...
def player_forming(cord):
    ...
    return Player(img=M6_Constants.IMG[string[0]], id=string[0], anim_radius=int(string[1].split(':')[1]), weapon=M6_Constants.SAVE_SHIP[1], cord=cord, health=data_inf[0], max_angle_speed=data_inf[1], speed_boost=data_inf[2])
