...
def line_bullet_shoot(perent, accuracy, range, damage):
    cord = '.'.join([str(int(i)) for i in perent.cord])
    return f'{str(random.randint(9999999, 100000000))}:{perent.id}:{cord}:{perent.angle}:{accuracy}:{range}:{damage}'